...
__host__ __device__ float Ray::has_intersection(const Sphere& sphere) const {
  const Vec3f sphere_to_origin = origin - sphere.center;

  // Coefficients for quadratic equation of the intersection of ray and sphere
  const float a = dot(dir, dir);
  const float b = dot((2.0f * (dir)), sphere_to_origin);
  const float c = dot(sphere_to_origin, sphere_to_origin) - sphere.radius * sphere.radius;
  ...
  const float sqrtVal = std::sqrt(d);

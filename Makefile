...
all: base static threads float pinned transfer divergence value shared
...
clean:
	rm -f $(OUT_BASE).out $(OUT_STATIC).out $(OUT_THREADS).out $(OUT_FLOAT).out $(OUT_PINNED).out $(OUT_TRANSFER).out $(OUT_DIVERGENCE).out $(OUT_VALUE).out $(OUT_SHARED).out
	rm -f $(OUT_BASE).nvvp $(OUT_STATIC).nvvp $(OUT_THREADS).nvvp $(OUT_FLOAT).nvvp $(OUT_PINNED).nvvvp $(OUT_TRANSFER).nvvp $(OUT_DIVERGENCE).nvvp $(OUT_VALUE).nvvp $(OUT_SHARED).nvvp
